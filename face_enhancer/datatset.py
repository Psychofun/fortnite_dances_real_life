...
class ImageFolderDataset(Dataset):
    ...
    def __init__(self, root, cache = None, is_test = False):
        ...
        if cache  is not None and os.path.isfile(cache):
            ...
            images_path = os.listdir(os.path.join(root, 'test_real'))
            self.images = sorted( images_path)
            self.root = root
            tmp = imread(os.path.join(images_path, self.images[0]) )
    ...
    def __getitem__(self,index):
        ...
        fake_img_path = os.path.join(self.root,'test_sync', name)
...
class FaceCropDataset(Dataset):
    ...
    def  get_full_sample(self, item):
        ...
        real_head = ( 
                        None if self.image_dataset.is_test else  
    ...
    def __getitem__(self, index):
        real_head, fake_head, _, _, _, _, _, _  = self.get_full_sample(index)
