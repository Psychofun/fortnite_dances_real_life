...
class Trainer(object):
    def __init__(self,ckpt_dir, log_dir,dataset, dataloader,
        ...
        self.recon_loss = VGG_perceptual_loss(preatined = True, device = self.device)
    ...
    def train_discriminator(self,g,d,d_opt):
        ...
        with torch.no_grad():
            ...
            ones.uniform(0.9,1.1)
            zeros.uniform(-0.1, 0.1)
        ...
        return dis_real_loss_val +  dis_fake_loss 
    ...
    def  train(self,generator,discriminator,batch):
        ...
        # Models to training mode. Compute gradients.
        generator.train()
        discriminator.train()
        ...
        while batch <= self.max_batches:
            ...
            if batch % self.log_every == 0:
                log_string =   "Batch {:d}".format(batch)

                for k,v  in log.items():
                    log_string += '[{}] {:5.3f}'.format(k,v)
                
                
                log_string += "Took {:5.2f}".format(time.time() - start)
                print(log_string)
                logs = self._init_logs()
                start = time.time()
